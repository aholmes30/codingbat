package array1;

import java.util.Arrays;

public class MakeLast {

    public static int[] makeLast(int[] nums) {

        // return data: int[]
        // param int[]

        // double the length of the array
        // make last element from original array last element in doubled array
        // all other elements in doubled array are 0 by default
        // orig array length will be >= 1

        int[] doubleArr = new int[nums.length * 2];
        doubleArr[nums.length * 2 - 1] = nums[nums.length - 1];
        return doubleArr;

    }

    public static void main(String[] args) {
        int[] nums = {6, 23, 12, 9};
        System.out.println(Arrays.toString(makeLast(nums)));
    }
}

/*

Given an int array, return a new array with double the length where its last element
is the same as the original array, and all the other elements are 0.
The original array will be length 1 or more. Note: by default, a new int array contains all 0's.

makeLast([4, 5, 6]) → [0, 0, 0, 0, 0, 6]
makeLast([1, 2]) → [0, 0, 0, 2]
makeLast([3]) → [0, 3]
 */